        ...
        if(root)
        {
            ...
            json_object_foreach(root, key, value)
            {
                if(json_is_integer(value) && !strcmp(key, THEMEPLAZA_JSON_PAGE_COUNT))
                    ...
                    throw_error("没有搜索内容的結果", ERROR_LEVEL_WARNING);
            }
        }
        else
            DEBUG("JSON文件于 %d: %s 有錯誤\n", error.line, error.text);

        json_decref(root);
    }
    else
        throw_error("无法下載主題广場数据\n請確保无線网已打開", ERROR_LEVEL_WARNING);
...
static void search_menu(Entry_List_s * list)
{
    ...
    swkbdSetHintText(&swkbd, "請選擇要搜索的標簽。");
    ...
    *buf = malloc(0x1000);
    ...
    if(filename)
    {
        ...
        if(!(tok))
        {
            ...
            free(new_url);
            free(*buf);
            throw_error("目標无效！!", ERROR_LEVEL_WARNING);
            DEBUG("filename\n");
            return 0;
        }
