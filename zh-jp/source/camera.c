...
bool start_capture_cam(qr_data *data) 
{
    ...
    {
        throw_error("捕獲凸輪線程創建失敗\n請將此信息報告給開發人員", ERROR_LEVEL_ERROR);
        return false; 
    }
        ...
        if (!quirc_decode(&code, &scan_data))
        {
            ...
            if(zip_size != 0)
            {
                ...
                if(r == ARCHIVE_OK)
                {
                    ...
                    {
                        throw_error("下載的壓縮包\n既不包含主題又不包含開機動畫.", ERROR_LEVEL_WARNING);
                    }
                }
                else
                {
                    throw_error("下載的文件不是壓縮包。", ERROR_LEVEL_WARNING);
                }
                free(zip_buf);
            }
            else
            {
                throw_error("下載失敗。", ERROR_LEVEL_WARNING);
            }
