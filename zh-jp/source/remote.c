...
static C2D_Image * load_remote_smdh(Entry_s * entry, bool ignore_cache)
{
    ...
    utf8_to_utf16(fallback_name, (u8*)"沒有名字", 0x80);
        ...
        if(root)
        {
            ...
            json_object_foreach(root, key, value)
            {
                if(json_is_integer(value) && !strcmp(key, THEMEPLAZA_JSON_PAGE_COUNT))
                    ...
                    throw_error("沒有搜索內容的結果", ERROR_LEVEL_WARNING);
            }
        }
        else
            DEBUG("JSON文件於 %d: %s 有錯誤\n", error.line, error.text);

        json_decref(root);
    }
    else
        throw_error("無法下載主題廣場數據\n請確保無線網已打開", ERROR_LEVEL_WARNING);
...
static void search_menu(Entry_List_s * list)
{
    ...
    swkbdSetHintText(&swkbd, "妳想搜索什麽標簽？");
    ...
    *buf = malloc(0x1000);
    ...
    if(filename)
    {
        ...
        if(!(tok))
        {
            ...
            free(new_url);
            free(*buf);
            throw_error("目標無效！!", ERROR_LEVEL_WARNING);
            DEBUG("filename\n");
            return 0;
        }
