...
void init_screens(void)
{
    ...
    C2D_TextParse(&text[TEXT_THEME_MODE], staticBuf, "主題模式");
    C2D_TextParse(&text[TEXT_SPLASH_MODE], staticBuf, "開機動畫模式");

    C2D_TextParse(&text[TEXT_NO_THEME_FOUND], staticBuf, "無主題");
    C2D_TextParse(&text[TEXT_NO_SPLASH_FOUND], staticBuf, "無開機動畫");

    C2D_TextParse(&text[TEXT_DOWNLOAD_FROM_QR], staticBuf, "按 \uE005 通過 QR Code 安裝");

    C2D_TextParse(&text[TEXT_SWITCH_TO_SPLASHES], staticBuf, "按 \uE004 切換到開機畫面界面");
    ...
    C2D_TextParse(&text[TEXT_THEMEPLAZA_THEME_MODE], staticBuf, "主題廣場 主題模式");
    C2D_TextParse(&text[TEXT_THEMEPLAZA_SPLASH_MODE], staticBuf, "主題廣場 開機動畫模式");

    C2D_TextParse(&text[TEXT_SEARCH], staticBuf, "搜索...");
    C2D_TextParse(&text[TEXT_PAGE], staticBuf, "頁碼:");

    C2D_TextParse(&text[TEXT_ERROR_QUIT], staticBuf, "按 \uE000 退出");
    C2D_TextParse(&text[TEXT_ERROR_CONTINUE], staticBuf, "按 \uE000 繼續");

    C2D_TextParse(&text[TEXT_CONFIRM_YES_NO], staticBuf, "\uE000 是   \uE001 否");

    C2D_TextParse(&text[TEXT_INSTALL_LOADING_THEMES], staticBuf, "正在讀取主題列表中，請稍等...");
    C2D_TextParse(&text[TEXT_INSTALL_LOADING_SPLASHES], staticBuf, "正在讀取開機動畫中，請稍等...");
    C2D_TextParse(&text[TEXT_INSTALL_LOADING_ICONS], staticBuf, "正在讀取圖標中，請稍等...");

    C2D_TextParse(&text[TEXT_INSTALL_SPLASH], staticBuf, "正在安裝開機動畫中...");
    C2D_TextParse(&text[TEXT_INSTALL_SPLASH_DELETE], staticBuf, "正在刪除開機動畫中...");

    C2D_TextParse(&text[TEXT_INSTALL_SINGLE], staticBuf, "正在安裝主題中...");
    C2D_TextParse(&text[TEXT_INSTALL_SHUFFLE], staticBuf, "正在安裝隨機主題組中...");
    C2D_TextParse(&text[TEXT_INSTALL_BGM], staticBuf, "正在安裝主題的BGM中...");
    C2D_TextParse(&text[TEXT_INSTALL_NO_BGM], staticBuf, "正在安裝主題圖片中...");

    C2D_TextParse(&text[TEXT_INSTALL_DOWNLOAD], staticBuf, "正在下載中...");
    C2D_TextParse(&text[TEXT_INSTALL_CHECKING_DOWNLOAD], staticBuf, "正在檢查下載的文件...");
    C2D_TextParse(&text[TEXT_INSTALL_ENTRY_DELETE], staticBuf, "正在從SD卡中刪除中...");

    C2D_TextParse(&text[TEXT_INSTALL_LOADING_REMOTE_THEMES], staticBuf, "正在下載主題列表中，請稍等...");
    C2D_TextParse(&text[TEXT_INSTALL_LOADING_REMOTE_SPLASHES], staticBuf, "正在下載開機列表中，請稍等...");
    C2D_TextParse(&text[TEXT_INSTALL_LOADING_REMOTE_PREVIEW], staticBuf, "正在下載預覽中，請稍等...");
    C2D_TextParse(&text[TEXT_INSTALL_LOADING_REMOTE_BGM], staticBuf, "正在下載BGM中，請稍等...");
