...
static void update_ui(void *arg)
{
    ...
    while(svcWaitSynchronization(data->event_stop, 2 * 1000 * 1000ULL) == 0x09401BFE) // timeout of 2ms occured, still have 14 for copy and render
    {
        ...
        set_screen(bottom);
        draw_text_center(GFX_BOTTOM, 4, 0.5, 0.5, 0.5, colors[COLOR_WHITE], /*"Press \uE005 To Quit"*/ "按 \uE005 退出");
        end_frame();
    }
...
static bool start_capture_cam(qr_data *data) 
{
    ...
    {
        throw_error(/*"Capture cam thread creation failed\nPlease report this to the developers"*/ "捕获进程创建失败\n请将此问题上报给开发者", ERROR_LEVEL_ERROR);
        LightEvent_Signal(&data->event_cam_info);
        LightEvent_Signal(&data->event_ui_info);
        return false;
    }
    ...
    if(finished && ready)
    {
        ...
        if(zip_size != 0)
        {
            ...
            if(r == ARCHIVE_OK)
            {
                ...
                {
                    throw_error(/*"Zip downloaded is neither\na splash nor a theme."*/ "下载的Zip文件不是开机动画及主题", ERROR_LEVEL_WARNING);
                }
            }
            else
            {
                throw_error(/*"File downloaded isn't a zip."*/ "下载的文件不是Zip", ERROR_LEVEL_WARNING);
            }
            free(zip_buf);
        }
        else
        {
            throw_error(/*"Download failed."*/ "下载失败", ERROR_LEVEL_WARNING);
        }
